 ...
 * socket.io2 bundle based on the url structure. Different transport names are
 * concatinated using the `+` char. /socket.io2/socket.io2+websocket.js should
...
Static.prototype.init = function () {
  ...
  /**
   * Generates a socket.io2-client file based on the supplied transports.
   *
   * @param {Array} transports The array with transport types
  ...
  this.add('/socket.io2.js', function (path, callback) {
    build(self.manager.get('transports'), callback);
  });

  this.add('/socket.io2.v', { mime: mime.js }, function (path, callback) {
    build(self.manager.get('transports'), callback);
  });

  // allow custom builds based on url paths
  this.add('/socket.io2+', { mime: mime.js }, function (path, callback) {
  ...
  this.manager.on('set:transports', function (key, value) {
    delete self.cache['/socket.io2.js'];
